...
public class TinyDB {
    ...
    /*public ArrayList<?> getObjectList(String key ,Class<?> object)
    {
        checkForNullKey(key);
        ArrayList<Object> playerObjects = getListObject(key, object);
        ArrayList<?> customerListSuitcases = new ArrayList<>();

        for(Object objs : playerObjects)
        {
            customerListSuitcases.add(objs);
        }
    ...
    public <T> T getObjectFromJson(String s , Class<T> tClass){
        Gson gson = new Gson();
